...
const _validateSignature = function (
    razorpay_payment_id,
    razorpay_order_id,
    secret_key
) {
...
const RegionServiceMock = {
    retrieve: jest
        .fn()
        .mockReturnValue(Promise.resolve({ currency_code: "INR" }))
};
const api_key = "WWW";
...
describe("RazorpayProviderService", () => {
    describe("createCustomer", () => {
        ...
        beforeAll(async () => {
            ...
            result = await razorpayProviderService.createCustomer(
                _.cloneDeep(test_customer)
            );
        ...
        it("returns created razorpay customer", () => {
            ...
            expect(result).toMatchObject({
                name:
                    test_customer.name?.substring(0, 50) ??
                    (
    ...
    describe("createOrder", () => {
        ...
        it("returns created razorpay payment intent for cart with existing customer", async () => {
            ...
            expect(
                result.offer_id === null || result.offer_id?.length > 0
            ).toBe(true);
            expect(result).toMatchObject({
            ...
            result = await razorpayProviderService.capturePayment({
                data: {
                    notes: {
                        ...
                        razorpay_signature: _validateSignature(
                            "pay_JFbzW5PV980gUH",
                            "order_JFapRWBCWCR3bx",
                            api_key_secret
                        )
                    }
                }
